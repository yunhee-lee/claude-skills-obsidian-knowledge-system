...
- 파일명은 `<책 제목> 저자명` 형식으로 자동 생성됨 (예: `<Slow Productivity> Cal Newport`)
...
1. `## Core Summary` 섹션 바로 아래가 비어있음 → **Stage 2 실행**
2. `## Understanding the Author's Argument` 섹션이 비어있음 → **Stage 3 실행**
3. `## Critical Review & Knowledge Expansion` 섹션이 비어있음 → **Stage 4 실행**
4. `## Life Application` 섹션에 적용 포인트가 비어있음 → **Stage 5 실행**
5. 모두 채워져 있음 → 완료 안내 출력

"비어있음" 판단 기준: 섹션 heading 바로 아래에 실질적인 내용(텍스트)이 없거나, 템플릿 placeholder만 있는 경우.
...
2. 파일 경로 결정:
   ```
   {{VAULT_PATH}}/0️⃣ Inbox/<책 제목> 저자명.md
   ```
3. 템플릿(`Primary Note - Book.md`)을 기반으로 노트 생성:
   - `{{title}}` → 책 제목
   - `{{author}}` → 저자명
...
- 파일: `0️⃣ Inbox/<책 제목> 저자명.md`
- 다음 단계: 책을 읽으면서 Reading Log에 기록하세요.
- 완독 후: `/book-reflection "<파일명>"` 을 실행하면 Stage 2부터 진행합니다.
```
...
**사용자가 반복적으로 반응한 것**
- [Reading Log에서 유독 많이 언급되거나 강하게 반응한 주제/구절 요약]
```

3. 작성한 내용을 `## Core Summary` 섹션에 삽입 (Edit tool 사용)
4. 사용자에게 확인 요청:
...
5. `## Understanding the Author's Argument` 섹션에 삽입
...
3. 관련된 노트 발견 시 사용자에게 제시:
   ...
   ```
4. `### Related Notes` 섹션에 관련 노트 링크 삽입
...
어떤 부분에서 저자가 지나치게 단순화하거나, 반례를 충분히 다루지 않은 것 같나요?
```

**Step B — 반대 관점 탐색**
AI가 강력한 반론을 제시: "이 주장에 반대하는 가장 강한 논거는 [X]입니다. 어떻게 생각하세요?"

**Step C — 연결된 사상가/책 제안**
AI가 이 주제에서 다른 관점을 가진 사상가나 책을 2-3개 제안:
```
이 주제와 관련해 탐색해볼 만한 것들:
- [저자/책]: [왜 관련되는지]
- [저자/책]: [왜 관련되는지]
```
...
대화 결과를 `## Critical Review & Knowledge Expansion` 섹션에 기록:
...
3. 사용자의 답변을 받으면 AI가 구체적인 적용 실험 설계를 도움:
   ...
   - 이번 주에 시도해볼 수 있는 것: [구체적인 행동]
   - 어떻게 되었는지 확인하는 방법: [관찰/측정 방법]
   ```
4. 1-2개의 적용 포인트를 확정하고 `## Life Application` 섹션에 기록:
...
이 책과 연결된 다른 책을 읽을 준비가 되면 다시 `/book-reflection init`으로 시작하세요.
```
...
- **파일을 찾을 수 없음**: 파일 경로 확인 요청. `0️⃣ Inbox/`에서 유사한 파일명 목록 제시
